...
describe("拍卖功能", function () {
  ...
  it("完整拍卖流程测试", async function () {
    ...
    // 设置价格预言机
    await auction.connect(deployerSigner).setPriceFeed(ethers.ZeroAddress, priceFeedAddress);
    console.log(`价格预言机设置成功: ${priceFeedAddress}`);
    
    // 验证价格预言机是否真的被设置了
    try {
      const priceFeedAnswer = await auction.getChainlinkDataFeedLatestAnswer(ethers.ZeroAddress);
      console.log("priceFeedAnswer:", priceFeedAnswer);
      console.log("价格预言机数据:", priceFeedAnswer.toString());
    } catch (error) {
      console.log("价格预言机验证失败:", error.message);
      
      // 尝试直接检查priceFeeds映射
      try {
        const priceFeedAddress = await auction.priceFeeds(ethers.ZeroAddress);
        console.log("priceFeeds[address(0)]:", priceFeedAddress);
        if (priceFeedAddress === "0x0000000000000000000000000000000000000000") {
          console.log("❌ 价格预言机确实没有被设置！");
        } else {
          console.log("✅ 价格预言机地址已设置，但调用失败");
        }
      } catch (mapError) {
        console.log("无法访问priceFeeds映射:", mapError.message);
      }
    }
